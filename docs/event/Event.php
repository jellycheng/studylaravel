<?php
namespace Jelly\Console\Scheduling;
...
class Event {
	 ...
	 * 使用哪个帐号执行命令 如 mysql，nobody，root
	 * @var string
	 */
	protected $user;
	...
	public function run(Container $container)
	{
		...
		{//执行多条命令
			$this->runCommandInForeground($container); //proc_open()
		}
		else
		{//在后台执行，一条命令
	...
	{
		//chdir(base_path());//切换到 base_path()=项目根目录

		exec($this->buildCommand()); // 执行一个外部程序
	}
	...
	{
		$command = $this->command.' > '.$this->output.' 2>&1 &'; //后台执行的命令： /abc/xxx > /dev/null 2>&1 & 
		// sudo -u jelly /abc/xxx > /dev/null 2>&1 &
