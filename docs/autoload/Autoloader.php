...
class Autoloader
{
    ...
    static public function autoload($class) {
        if (0 === strpos($class, 'PhpParser\\')) {#以PhpParser\开头的类名且区分大小写 使用这种方式加载类文件，
            $fileName = dirname(__DIR__) . '/' . strtr($class, '\\', '/') . '.php';#当前文件同级目录/PhpParser/命名空间目录/类名.php
            if (file_exists($fileName)) {
                require $fileName;
            }
        } else if (0 === strpos($class, 'PHPParser_')) {#以PHPParser_开头的类 则设置类别名，类文件不存在则会触发自动加载器
    ...
    private static function registerLegacyAliases() {
        foreach (self::$oldToNewMap as $old => $new) {
            class_alias($new, $old);# bool class_alias ( string $original原类 , string $alias别名 [,bool $autoload =TRUE是否原类不存在自动加载 ])基于用户定义的类 original 创建别名 alias。 这个别名类和原有的类完全相同
        }
    }
